// ==UserScript==
// @name         DevScope 开发者工具 v5.4.5
// @namespace    devtools-sidebar-native
// @version      5.4.5
// @description  完整独立调试工具 - 百级缩放/精调按钮/底部不遮挡
// @author       Developer
// @run-at       document-end
// @match        *://*/*
// @grant        none
...
(function() {
    ...
    const SVG = {
        ...
        pin: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2 L12 22 M5 9 L12 2 L19 9"/></svg>'
    };
    ...
    let visualWidth = parseInt(localStorage.getItem('devtools-visual-width')) || 420;
    let sidebarMode = localStorage.getItem('devtools-sidebar-mode') || 'push';
    let sidebarZoom = parseFloat(localStorage.getItem('devtools-sidebar-zoom')) || 1;
    let sidebarOpacity = parseFloat(localStorage.getItem('devtools-sidebar-opacity')) || 1;
    let autoShowInspector = localStorage.getItem('devtools-auto-inspector') === 'true';
    ...
    function createStyles() {
        ...
        style.textContent = `
            .devtools-toggle-btn {
                ...
                z-index: 2147483647; background: #0078d4; color: #fff; border: none;
                padding: 10px 6px; border-radius: 6px 0 0 6px; cursor: pointer; font-size: 20px;
                box-shadow: -2px 0 12px rgba(0,0,0,0.3); font-family: sans-serif; line-height: 1;
                text-align: center; width: 32px; transition: right 0.3s, background 0.2s;
                animation: devtools-pulse 2s infinite; pointer-events: auto;
            }
            .devtools-toggle-btn:hover { background: #005a9e; }
            ...
            .devtools-sidebar {
                ...
                z-index: 2147483646; transition: right 0.3s; display: flex; flex-direction: column;
            ...
            .devtools-tree-node:hover { background: var(--bg-hover, #f5f5f5); }
            .devtools-tree-node.selected { background: var(--bg-selected, #000); color: #fff; }
            ...
            .devtools-tag-name { color: var(--text-tag, #555); }
            .devtools-attr-name { color: var(--text-attr, #666); }
            .devtools-attr-value { color: var(--text-primary, #333); }
            ...
            .devtools-highlight-overlay {
                ...
                background: rgba(0,0,0,0.1); border: 2px solid var(--text-primary, #000);
            ...
            .devtools-network-method { min-width: 50px; font-weight: bold; }
            .devtools-network-url { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--text-secondary, #666); }
            ...
            .devtools-context-menu {
                position: fixed; z-index: 2147483648; background: var(--bg-panel, #fff);
            ...
            .devtools-settings-panel {
                ...
                z-index: 10; min-width: 320px; font-size: 12px;
            }
    ...
    function createSidebar() {
        ...
        header.innerHTML = `
            ...
            <div class="devtools-header-right">
                <button class="devtools-theme-btn" id="devtools-pin-btn" title="强制置顶">${SVG.pin}</button>
            ...
            <div class="devtools-settings-panel" id="devtools-settings-panel">
                ...
                <div class="devtools-setting-row">
                    <span class="devtools-setting-label">面板宽度<small>视觉宽度(px)</small></span>
                    <div class="devtools-setting-control">
                        <input type="range" class="devtools-setting-range" id="devtools-setting-width" min="300" max="800" step="10" value="${visualWidth}">
                        <span class="devtools-setting-range-val" id="devtools-setting-width-val">${visualWidth}px</span>
                    </div>
                </div>
                <div class="devtools-setting-row">
                    <span class="devtools-setting-label">内容缩放<small>字体/控件 (0.50~1.50)</small></span>
                    <div class="devtools-setting-control">
        ...
        elementsPanel.innerHTML = `
            <div class="devtools-elements-wrapper">
                <div class="devtools-elements-header">
                    ...
                    <button class="devtools-console-btn" id="devtools-copy-source" title="复制当前页面完整源码">复制源码</button>
                </div>
    ...
    function initEventListeners() {
        document.getElementById('devtools-pin-btn').addEventListener('click', forceTopMost);
        ...
        document.getElementById('devtools-setting-mode').addEventListener('change', e => {
            ...
            localStorage.setItem('devtools-sidebar-mode', sidebarMode);
        ...
        document.getElementById('devtools-setting-width').addEventListener('input', e => {
            ...
            localStorage.setItem('devtools-visual-width', visualWidth);
            applySidebarSettings();
        });
        document.getElementById('devtools-setting-zoom').addEventListener('input', e => {
            ...
            localStorage.setItem('devtools-sidebar-zoom', sidebarZoom);
            applySidebarSettings();
        });
        ...
        document.getElementById('devtools-setting-opacity').addEventListener('input', e => {
            ...
            localStorage.setItem('devtools-sidebar-opacity', sidebarOpacity);
            applySidebarSettings();
        });
        ...
        document.getElementById('devtools-copy-source').addEventListener('click', copyPageSourceWithHighlight);
    ...
    function adjustZoom(delta) {
        ...
        localStorage.setItem('devtools-sidebar-zoom', newZoom);
        applySidebarSettings();
    }
    ...
    function forceTopMost() {
        ...
        topMostTimer = setTimeout(() => {
            ...
            if (!topMostObserver) {
                topMostObserver = new MutationObserver(mutations => {
                    for (const m of mutations) {
                        for (const node of m.addedNodes) {
                            if (node.nodeType === 1 && node !== sidebar && node !== toggleBtn) {
                                const st = getComputedStyle(node);
                                if ((st.position === 'fixed' || st.position === 'absolute') && parseInt(st.zIndex) >= 2147483640) {
                                    forceTopMost();
                                    return;
                                }
                            }
                ...
                topMostObserver.observe(document.body, { childList: true });
            }
        }, 50);
    }
    ...
    function renderTreeNode(element, level) {
        ...
        totalNodesRendered++;
        const hasChildren = element.children && element.children.length > 0 && level < 15;
    ...
    function copyPageSourceWithHighlight() {
    ...
    window.devtoolsTestRegex = function() {
        ...
        if (!pattern || !text) { resultDiv.innerHTML = '<div class="devtools-result-box error">请输入正则表达式和测试文本</div>'; return; }
        try {
            let regex;
            if (pattern.startsWith('/') && pattern.lastIndexOf('/') > 0) {
                const parts = pattern.match(/^\/(.+)\/([gimsuy]*)$/);
                regex = parts ? new RegExp(parts[1], parts[2]) : new RegExp(pattern);
    ...
    window.devtoolsTestSelector = function() {
        ...
        if (!input) { resultDiv.innerHTML = '<div class="devtools-result-box error">请输入选择器</div>'; return; }
        try {
            let elements = [];
            ...
            resultDiv.innerHTML = html;
        } catch (e) { resultDiv.innerHTML = '<div class="devtools-result-box error">选择器错误: ' + escapeHtml(e.message) + '</div>'; }
    };
    ...
    window.devtoolsFormatJson = function() {
        ...
        } catch (e) { resultDiv.innerHTML = '<div class="devtools-result-box error">JSON解析错误: ' + escapeHtml(e.message) + '</div>'; }
    };
    ...
    window.devtoolsCopyJson = function() {
        if (window.__formattedJson) {
            ...
            const input = document.getElementById('devtools-json-input').value;
            try { const parsed = JSON.parse(input); navigator.clipboard.writeText(JSON.stringify(parsed, null, 2)).then(() => alert('已复制！')).catch(() => alert('复制失败')); } catch (e) { alert('无效的JSON'); }
        }
    };
    ...
    window.devtoolsStringOp = function(op) {
        ...
        } catch (e) { resultDiv.innerHTML = '<div class="devtools-result-box error">操作失败: ' + escapeHtml(e.message) + '</div>'; }
    };
    ...
    window.devtoolsSendRequest = async function() {
        ...
        if (!url) { responseDiv.innerHTML = '<div class="devtools-result-box error">请输入URL</div>'; return; }
        responseDiv.innerHTML = '<div class="devtools-result-box">正在发送请求...</div>';
        let headers = {};
        try { if (headersText.trim()) headers = JSON.parse(headersText); } catch (e) { responseDiv.innerHTML = '<div class="devtools-result-box error">请求头格式错误: ' + escapeHtml(e.message) + '</div>'; return; }
        ...
        try {
            ...
            responseDiv.innerHTML = '<div class="devtools-details-tabs"><div class="devtools-details-tab active" onclick="window.devtoolsSwitchReqRespTab(\'formatted\',this)">格式化</div><div class="devtools-details-tab" onclick="window.devtoolsSwitchReqRespTab(\'raw\',this)">原始</div></div><div id="devtools-reqresp-tab-formatted"><div style="margin-bottom:8px;padding:8px;background:#f5f5f5;border-radius:3px;font-size:11px;"><div><strong>状态:</strong> ' + response.status + ' ' + response.statusText + '</div><div><strong>耗时:</strong> ' + duration + 'ms</div><div><strong>大小:</strong> ' + responseText.length + ' 字节</div></div><pre style="padding:8px;background:#f5f5f5;border:1px solid #eee;border-radius:3px;font-size:10px;white-space:pre-wrap;word-break:break-all;max-height:300px;overflow:auto;">' + escapeHtml(formattedText) + '</pre></div><div id="devtools-reqresp-tab-raw" style="display:none;"><pre style="padding:8px;background:#f5f5f5;border:1px solid #eee;border-radius:3px;font-size:10px;white-space:pre-wrap;word-break:break-all;max-height:300px;overflow:auto;">' + escapeHtml(responseText) + '</pre></div>';
        } catch (e) { responseDiv.innerHTML = '<div class="devtools-result-box error">请求失败: ' + escapeHtml(e.message) + '</div>'; }
    };
    ...
    function getCookies() {
        try { if (!document.cookie) return []; return document.cookie.split(';').map((cookie, idx) => { const parts = cookie.trim().split('='); return { key: parts[0], value: parts.slice(1).join('='), id: idx }; }).filter(item => item.key); } catch(e) { return []; }
    }
    ...
    function renderStorageTable(data, title, type) {
        ...
        html += '</tbody></table>';
        if (['local', 'session'].includes(type)) html += '<button class="devtools-add-row" onclick="window.devtoolsAddStorage(\'' + type + '\')">+ 添加</button>';
        container.innerHTML = html;
    }
    ...
    window.devtoolsEditStorage = function(key, type) {
        ...
        if (newValue !== null) { if (type === 'local') localStorage.setItem(key, newValue); else sessionStorage.setItem(key, newValue); renderResourceContent(); }
    };
    ...
    window.devtoolsDeleteStorage = function(key, type) {
        if (confirm('确定删除 "' + key + '" 吗?')) { if (type === 'local') localStorage.removeItem(key); else sessionStorage.removeItem(key); renderResourceContent(); }
    };
    ...
    function renderUserAgent() {
        ...
        container.innerHTML = '<div class="devtools-resource-header"><span class="devtools-resource-title">User Agent</span></div><div style="padding:8px;font-size:11px;"><div style="margin-bottom:10px;padding:10px;background:#f5f5f5;border-radius:4px;word-break:break-all;font-family:Consolas,monospace;font-size:10px;">' + escapeHtml(ua) + '</div><button class="devtools-action-btn" onclick="window.devtoolsCopyUA()">复制</button></div>';
    }
    window.devtoolsCopyUA = function() {
        if (window.__uaString) navigator.clipboard.writeText(window.__uaString).then(() => alert('已复制')).catch(() => alert('复制失败'));
    };
    ...
    function renderPerformanceContent() {
        try {
            ...
            container.innerHTML = '<div class="devtools-performance-metric"><span>加载时间</span><span>' + loadTime + '</span></div><div class="devtools-performance-metric"><span>DOM解析</span><span>' + domParse + '</span></div><div class="devtools-performance-metric"><span>首字节</span><span>' + firstByte + '</span></div><div class="devtools-performance-metric"><span>节点数</span><span>' + document.querySelectorAll('*').length + '</span></div>';
        } catch (e) {}
    }
    ...
    function init() {
        ...
        hijackNetwork();
        addKeyboardShortcut();
        console.log('DevScope v5.4.5 已加载');
    }
