...
(function() {
    ...
    let sidebarZoom = parseFloat(localStorage.getItem('devtools-sidebar-zoom')) || (isMobile ? 0.9 : 1);
    ...
    function renderNetworkRequests() {
        ...
        const filtered = networkRequests.filter(req => {
            if (typeFilter !== 'all') {
                ...
                if (typeFilter === 'CSS' && ext !== 'css') return false;
                if (typeFilter === 'Img' && !['png','jpg','jpeg','gif','svg','webp','ico'].includes(ext)) return false;
    ...
    window.devtoolsStringOp = function(op) {
        ...
        try {
            switch (op) {
                ...
                case 'sha256': window.devtoolsSha256(input).then(r => { resultDiv.innerHTML = '<div class="devtools-result-box"><pre style="margin:0;word-break:break-all;font-size:10px;">' + escapeHtml(r) + '</pre></div><button class="devtools-console-btn" onclick="window.devtoolsCopyToolResult(\'devtools-string-result\')" style="margin-top:8px;">复制结果</button>'; }); return;
